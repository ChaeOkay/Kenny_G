...
describe SanitizedOptions do
  xit 'raises error if no name is provided' do
    options = { winning_score: 100, players: nil }
    expect{ described_class.new(options) }
      .to raise_error(Errors::NoPlayerName)
  end

  xit 'raises error if name is an empty string' do
    options = { winning_score: 100, players: '' }
    expect{ described_class.new(options) }
      .to raise_error(Errors::NoPlayerName)
  end

  xit 'raises error if winning score is less than 0' do
  ...
  context 'valid options' do
    ...
    describe '#instance_values' do
      xit 'returns a hash of ivars as symbols with value as the key, railsy' do
